...
def PreStepUserFunction(mbs, t):
    ...
    if config.simulation.changeActorStiffness:
        for itemNumber in Actors:
            ChangeActorStiffnessStartingNode(mbs,t,itemNumber,ADE)
    ...
    if config.simulation.useInverseKinematic:
        for i in range(len(mbs.variables['springDamperDriveObjects'])):
            # values are in     
            if mbs.variables['correction']: 
                u1 = mbs.variables['lenCorrection0'][i]/10000
                u2 = mbs.variables['lenCorrection1'][i]/10000
            else: 
...
while not startSimulation:
    try:
        ...
        mbs.variables['nodeToChangeActorStiffness'] = []
        ...
        if startSimulation and (type(adeNodes) == df.ElementDefinition or type(adeNodes) == dict):
            ...
            # sensorTCP = mbs.AddSensor(SensorMarker(markerNumber=mbs.GetSensor(ADE[3]['sensors'][0])['markerNumber'], outputVariableType=exu.OutputVariableType.Position)) #530  
            # sensorTCP2 = mbs.AddSensor(SensorMarker(markerNumber=mbs.GetSensor(ADE[3]['sensors'][1])['markerNumber'], outputVariableType=exu.OutputVariableType.Position)) #530  
            # sensorTCP3 = mbs.AddSensor(SensorMarker(markerNumber=mbs.GetSensor(ADE[3]['sensors'][2])['markerNumber'], outputVariableType=exu.OutputVariableType.Position)) #530  
            sensorTCP = mbs.AddSensor(SensorMarker(markerNumber=mbs.GetSensor(ADE[6]['sensors'][0])['markerNumber'], outputVariableType=exu.OutputVariableType.Position)) #530  
            sensorTCP2 = mbs.AddSensor(SensorMarker(markerNumber=mbs.GetSensor(ADE[6]['sensors'][1])['markerNumber'], outputVariableType=exu.OutputVariableType.Position)) #530  
            sensorTCP3 = mbs.AddSensor(SensorMarker(markerNumber=mbs.GetSensor(ADE[6]['sensors'][2])['markerNumber'], outputVariableType=exu.OutputVariableType.Position)) #530  
            
            mbs.variables['nodeToChangeActorStiffness'] = sensorTCP
            ##+++++++++++++++++++++++++++++++++++++++
            ## Assemble Mesh 
            mbs.Assemble()
            ...
            ## Starting Simulation without Movelist
            elif config.simulation.useInverseKinematic == True:
                ...
                ikObj = InverseKinematicsATE(mbs2, [sensorTCP, sensorTCP2, sensorTCP3], mbs, oGround,ADE) # 66 is the Node  on the most upper right ...    
                # ikObj = InverseKinematicsATE(mbs2, [sensorTCP], mbs, oGround) # 66 is the Node  on the most upper right ...    
                ...
                if 1: 
                    ...
                    for j in range(len(phi)-1): 
                        ...
                        dx = xNew - xOld
                        ikObj.InverseKinematicsRelative(None, np.array(list(dx)+list(dx)+list(dx)))
                ...
                for i in range(len(sortedActorStrokesList)-1):
                    ...
                    if computeDynamic:
                        ...
                        #     mbs.systemData.SetSystemState(sysStateList,configuration = exu.ConfigurationType.Initial)
                        #     if 0: 
                        #         ikObj.InverseKinematicsRelative(mbs, [0., 0.1])
                                
                        #         p =  []
                        #         aL = []
                        #         for mbs_i in [mbs, ikObj.mbsIK]: 
                        #             actorLength = []
                        #             for j, oSpringDamper in enumerate(mbs_i.variables['springDamperDriveObjects']): 
                        #                 actorLength += [ mbs_i.GetObjectOutput(oSpringDamper,  exu.OutputVariableType.Distance, )] #- ikObj.l0[i]
                        #             p += [mbs_i.GetNodeOutput(66, exu.OutputVariableType.Position)]
                        #             aL += [actorLength]
                        #             print('actor lengths: ', np.round(actorLength, 3)*10000)
                        #             print('node 66 position: ', p[-1])
                        #         print('offset driven: ', np.diff(p, axis=0))
                        #     else: 
                        #         xOld = r* np.array([cos(phi[i]), np.sin(phi[i])])
                        #         xNew = r* np.array([cos(phi[i+1]), np.sin(phi[i+1])])
                        #         dx = xNew - xOld
                        #         ikObj.InverseKinematicsRelative(mbs, dx)
                                    
                        #     actorLengthCorrected = ikObj.GetActuatorLength()
                                
                        #     diffActuatorlength = actorLengthCorrected - np.array(sortedActorStrokesList[cnt])
                        #     sortedActorStrokesList[cnt+1] += diffActuatorlength            
                        #     mbs.variables['lenCorrection0'] = np.array(sortedActorStrokesList[cnt]) # in m
                        #     mbs.variables['lenCorrection1'] =  np.array(actorLengthCorrected)*10000 # np.array(sortedActorStrokesList[cnt+1])*0 +  # in m
                        #     sortedActorStrokesList[cnt+1] =  (np.array(actorLengthCorrected)*10000).tolist() # in 1/10 mm
                        ## Sending new Sensor Values to Webserver
                        SensorValuesTime[cnt+1][0]={}
                        SensorValues[cnt+1] = {}
                        for ID in adeIDs:
