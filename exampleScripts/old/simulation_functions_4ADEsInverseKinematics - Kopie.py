#+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
# This is an ARDS Python Utility library
#
# Details:  simulation_functions.py
#
# Author:   Emanuel Ulrich
# Date:     2021-06-30
#
# Copyright: ---
#
...
def createSimplifiedModel(mbs, adeMoveList2=None):
    ...
    sensorMbs2TCP = mbs2.AddSensor(SensorMarker(markerNumber=mbs2.GetSensor(ADE2[3]['sensors'][0])['markerNumber'], outputVariableType=exu.OutputVariableType.Position)) #530  
    mbs2.variables['sensorTCP']=sensorMbs2TCP
    
    sensorMbs2TCP2 = mbs2.AddSensor(SensorMarker(markerNumber=mbs2.GetSensor(ADE2[3]['sensors'][1])['markerNumber'], outputVariableType=exu.OutputVariableType.Position)) #530  
    sensorMbs2TCP3 = mbs2.AddSensor(SensorMarker(markerNumber=mbs2.GetSensor(ADE2[3]['sensors'][2])['markerNumber'], outputVariableType=exu.OutputVariableType.Position)) #530  
    ...
    ikObj = InverseKinematicsATE(mbs2, [sensorMbs2TCP], mbs, oGround2, ADE2) # 66 is the Node  on the most upper right ...
                  
    return ikObj, mbs2, sensorMbs2TCP
...
while not startSimulation:
    try:
        ...
        config.simulation.useCompliantJoints=True          #False for ideal
        config.simulation.numberOfElementsBeam=8
        config.simulation.numberOfElements=16
        ...
        if startSimulation and (type(adeNodes) == df.ElementDefinition or type(adeNodes) == dict):
            ...
            ## Starting Simulation without Movelist
                ...
                ikObj, mbs2, sensorMbs2TCP = createSimplifiedModel(mbs)
                ...
                dxx=[np.array([0.01,0]),np.array([0.01,0])]
                dxx=[np.array([0.03,0])]
                ...
                for k in range(len(dxx)):
                    ...
                    dx=list(dxx[k]) #<- change value here to go

                    ikObj.InverseKinematicsRelative(None, np.array(list(dx)))
                    iWantToGoToThisValue = mbs2.GetSensorValues(sensorMbs2TCP)[0:2]
                    ...
                    for i in range(stepsToCorrect):
                        ...
                        if computeDynamic:
                            ...
                            if correctTrajectory:
                                dx=[0,0]
                                # dx = iWantToGoToThisValue  - mbs.GetSensorValues(sensorTCP)[0:2]
                                # actorList=GetActuatorLength(mbs)
                                
                                # if i==stepsToCorrect-1:
                                #     print('laststep:',i)
                                #     dx = iWantToGoToThisValue  - mbs.GetSensorValues(sensorTCP)[0:2] #mbs.GetSensorValues(sensorTCP)[0:2]
                                    
                                    
                                # averageSensorValues=AverageMeshPoints(SensorValues[cnt+1],adeNodes,draw = True)
                                # averageSensorValues=SensorValues[cnt+1]
                                # actorList2=CalculateActorLengths(averageSensorValues)
                                
                                # new = CalculateNextMovementCommandsFromSensor(actorList2, adeMoveList)
                                ...
                                # adeMoveList = df.adeMoveList()
                                # adeNodes = df.ElementDefinition()    
                                # adeNodes.nodeDefinition[1] = np.array(averageSensorValues[4][1])[0]
                                # adeNodes.nodeDefinition[2] = np.array(averageSensorValues[4][2])[0]
                                # adeNodes.nodeDefinition[3] = np.array(averageSensorValues[4][0])[0]
                                # adeNodes.nodeDefinition[4] = np.array(averageSensorValues[1][2])[0]
                                # adeNodes.nodeDefinition[5] = np.array(averageSensorValues[2][2])[0]
                                # adeNodes.nodeDefinition[6] = np.array(averageSensorValues[3][0])[0]
                                ...
                                # ADE2, mbs2 = GenerateMesh(adeNodes,mbs)
                                # ikObj, mbs2, sensorMbs2TCP = createSimplifiedModel(mbs)

                                # mbs2.Assemble()       
                                    # adeMoveList.data[0]=adeMoveList.data[1]
                                #     ikObj, mbs2, sensorMbs2TCP = createSimplifiedModel(mbs, new)
                                
                                ikObj.InverseKinematicsRelative(mbs, np.array(list(dx)))
