    ...
    match myState:
        ...
        case gripperStates.APPROACH_OBJECT:
            ...
            if 'trajApproach' not in context:
                ...
                startLen = ((np.array(ikObj.GetActuatorLength()) - refLength) * 10000)
                
                targetPos = np.array([0.229, 0.1])
                targetAng = 0
                ...
                traj = tf.syncPTPTrajectory(startLen, targetLen, vMax, aMax)                
...
# Funktion, welche eine trajektorie abfährt
def runTrajectorySimulation(traj, context):
    t_sim = 0
    dt = 2
    refLength = context['refLength']
    sortedActorStrokesList = context['sortedActorStrokesList']
    ...
    mbs = context['mbs']
    timeTotal = context['timeTotal']
    numberOfSteps = context['numberOfSteps']
    SensorValuesTime = context['SensorValuesTime']
    SensorValues = context['SensorValues']
    SensorValues2 = context['SensorValues2']
    SensorValuesTime2 = context['SensorValuesTime2']
    cnt = context['cnt']
    counterSensorPlot = context['counterSensorPlot']
    ADE = context['ADE']
    adeIDs = context['adeIDs']
    cnt = context['cnt']
        ...
        try:
            ...
            if useInverseKinematic:
                ...
                if config.simulation.useInverseKinematic == True:
                    ...
                    # approachPos=np.array([objPos[0], objPos[1]-0.7])
                    # approachAng=0
                    # # approachPos=np.array([0.3, 0.1])
                    # # approachAng=-np.pi/16
                    # trajPositions.append(targetToNodeListCoords(approachPos, approachAng, context['TCPOffsets']))

                    # grippingPos=np.array([0, 0.4])
                    # grippingAng=0
                    # trajPositions.append(targetToNodeListCoords(grippingPos, grippingAng, context['TCPOffsets']))
                    ...
                    #     trajVec.append(tf.syncPTPTrajectory(prevActorLengths, actorLengths, vmax, amax))
                    #     prevActorLengths = actorLengths
                    
                    # time all trajactories need in total
                    # tTotal = sum(traj.t_total for traj in trajVec)
                    
                    # visualize actuator positions over time
